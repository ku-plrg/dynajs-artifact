...
test_suite("---------- Testing str.substring() -------- ", function () {
    ...
    let tainted = "lo";
    let c = a.substring(3); // "lo, World!"
    let d = a.substring(6, 3); // "lo,"
    ...
    test_one("substring with omitted end should preserve tainted prefix", function () {
        __assert_string_range_all_tainted__(c, 0, tainted.length);
    });

    test_one("substring with omitted end should preserve untainted suffix", function () {
        __assert_string_range_all_untainted__(c, tainted.length, c.length);
    });

    test_one("substring with swapped args should preserve tainted prefix", function () {
        __assert_string_range_all_tainted__(d, 0, tainted.length);
    });

    test_one("substring with swapped args should preserve untainted suffix", function () {
        __assert_string_range_all_untainted__(d, tainted.length, d.length);
    });

    test_one("substring with negative and oversized indices should preserve tainted prefix", function () {
        __assert_string_range_all_tainted__(e, 0, 5);
    });

    test_one("substring with negative and oversized indices should preserve untainted suffix", function () {
        __assert_string_range_all_untainted__(e, 5, e.length);
    });

    test_one("substring with undefined and NaN bounds should produce untainted empty string", function () {
        __jalangi_assert_taint_false__(f);
    });

    test_one("substring with NaN start should coerce start to zero", function () {
        __assert_string_range_all_tainted__(g, 0, g.length);
    });
