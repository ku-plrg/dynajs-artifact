...
import { getTaintEntry, anyPropertiesTainted, initPropMap, oid, getValue, FlowError } from "../Taint";
import { registerGhostFunction } from "../GhostFunction";
import { BaseContext } from "./BaseContext";
import { inspect } from "../Trace";
import { describePath, newPathNode, get_untainted_vals, PathNode, get_number_of_nodes, get_tainted_vals } from "../TaintPaths";
...
export function TEvalPolicy(s: State, code: Wrapped): Either<State, Error> {
    ...
    let sP = setMt(s, MtP);
    if (EVALSINK) {
        s.instr.trace_prop.called_sink = 'eval';
        if (s.instr.trace_prop.provenance_complexity == 0){
            s.instr.trace_prop.provenance_complexity = get_number_of_nodes(Array.from(F.eitherThrow(getTaintEntry(s, code)).path.parents)[0]);
        }
        
        if (s.instr.trace_prop.prefix_ace === ""){
            s.instr.trace_prop.prefix_ace = get_untainted_vals(Array.from(F.eitherThrow(getTaintEntry(s, code)).path.parents)[0]);
        }
        var err = new FlowError(`Sink function eval reached with tainted argument ${inspect(code)}\n`, s);
        if (tE.taintBit) {
            s.instr.trace_prop.triggers_flow = 1;
            s.instr.trace_prop.attacker_controlled_data = get_tainted_vals(Array.from(F.eitherThrow(getTaintEntry(s, code)).path.parents)[0], 'eval');
