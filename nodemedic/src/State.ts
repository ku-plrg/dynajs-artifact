...
export interface Frame {
    ...
    // Caller context to restore when this frame is resumed.
...
export interface State {
    // Backwards ref to instrumentation object
    instr: any,
    // The current frame
    frame: Frame,
    // Mw ::= . | Mw, (ref o) -> (id, p)
...
export function initFrame(callType: CallType, callerIID: IID): Frame {
    let frame: Frame = {
        ...
        savedContext: Context.Unset,
    };
    return frame;
}

export function initState(instr: any): State {
    let s: State = {
        instr: instr,
...
export function setIDS(s: State, IDS: Stack<ID>): State {
    let fP: Frame = {
        ...
        savedContext: s.frame.savedContext,
    }
    let sP: State = {
        instr: s.instr,
        frame: fP,
