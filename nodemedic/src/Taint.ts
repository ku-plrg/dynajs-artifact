...
import { emptyPathNode, newPathNode, describePath, circularReplacer, get_untainted_vals, get_number_of_nodes, get_tainted_vals } from './TaintPaths';
import { Wrapped } from './Wrapper';
import { TraceProperty } from './Base';
...
export class FlowError extends Error {
  ...
  constructor(message: string, s: State) {
    super(message);
    this.found_flow = true;
    this.trace_prop = s.instr.trace_prop.clone();
...
export function TCheck(s: State, v: Object) {
    ...
    if (tainted) {
                    if (s.instr.taint_path !== "disabled"){
        describePath(F.eitherThrow(getTaintEntry(s, v)).path);
            }
            
        var err = new FlowError(`Tainted argument found`, s);
    ...
    for (let i in args) {
        ...
        let argTEP = setPath(
            ...
            newPathNode(
                ...
                getValue(s, args[i]),
                getTc(s),
                F.maybeToNullable(SINKS.get(cF)) ?? (Object.getPrototypeOf(cF) === Function ? 'Function' : null)
            )
        );
 ...
 */
export function hitFuncSink(s: State, x: Wrapped, base: Wrapped, args: any): Either<State, Error> {
    var old_args = args;
    ...
    if (xP === Function.prototype.apply || xP === Function.prototype.call) {
        cF = base;
        if (xP === Function.prototype.apply){
            args = args[1];
        }
    }
    ...
    SINKS.forEach((name_func: string, f: Function) => {
        // Match Function subclass constructors (AsyncFunction, GeneratorFunction, etc.)
        if (f === cF || (f === Function && Object.getPrototypeOf(cF) === Function)) {
            s.instr.trace_prop.called_sink = name_func;
            var must_be_tainted = 0;
            if (name_func === "Function"){
                must_be_tainted = args.length - 1;
            }

            if (s.instr.trace_prop.provenance_complexity == 0){
                s.instr.trace_prop.provenance_complexity = get_number_of_nodes(F.eitherThrow(getTaintEntry(s, args[must_be_tainted])).path);
            }
            if (f === _SINKS[0][0]){
                if (s.instr.trace_prop.prefix_ace === ""){
                    s.instr.trace_prop.prefix_ace = get_untainted_vals(F.eitherThrow(getTaintEntry(s, args[must_be_tainted])).path);
                }
            }
            if (hasTaintedArg) {
		        s.instr.trace_prop.triggers_flow = 1;
                if (!taintedIdx.includes(must_be_tainted)){ // Function must have last argument tainted
                        s.instr.trace_prop.triggers_flow = 0.3;
                }

                s.instr.trace_prop.attacker_controlled_data = get_tainted_vals(F.eitherThrow(getTaintEntry(s, args[must_be_tainted])).path, name_func);
                for (let i in taintedIdx) {
                    if (s.instr.taint_path !== "disabled"){
                        if (xP === Function.prototype.apply){
                            describePath(F.eitherThrow(getTaintEntry(s, old_args[1])).path, i);
                        }
                        else{
                ...
                var err = new FlowError(`Sink1 ${f} reached with tainted arguments ${argsToString(args, (x) => taintedIdx.includes(x))}\n`, s);
                setTimeout(() => {
                    throw err;
                });
                throw err;
