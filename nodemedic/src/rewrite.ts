...
(function (sandbox) {
    ...
    function MyAnalysis() {
        ...
        function getBranchInfo() {
            var branchInfo = instrumentation.trace_prop.branches[sandbox.sid - 1];
            if (!branchInfo) {
                branchInfo = {};
                instrumentation.trace_prop.branches[sandbox.sid - 1] = branchInfo;
            }
            return branchInfo;
        }

        function getGlobalBranchInfo() {
            var branchInfo = instrumentation.trace_prop.global_branches[sandbox.sid - 1];
            if (!branchInfo) {
                branchInfo = {};
                instrumentation.trace_prop.global_branches[sandbox.sid - 1] = branchInfo;
            }
            return branchInfo;
        }
        ...
        this.invokeFunPre = function (iid, f, base, args, isConstructor, isMethod, functionIid, functionSid) {
            ...
            if (f !== undefined) {
                ...
                let ret = instrumentation.invokeFunPre(f, base, args, isMethod, isConstructor, isExternal, uniqueID, originalScriptPath, sourceLocation);
