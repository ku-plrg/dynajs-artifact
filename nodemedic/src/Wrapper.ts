...
function saveFrame(s: State, callType: CallType, iid: IID): State {
    ...
    let Fp = F.matchMaybe(s.F.get(iid), {
        Just: (frameBucket: Stack<Frame>) => {
            let frameBucketP = frameBucket.push(frameToSave);
            return s.F.set(iid, frameBucketP);
        },
        Nothing: () => {
            let l: List<Frame> = List();
            let lP = l.push(frameToSave);
...
function restoreFrame(s: State, iid: IID): State {
    return F.matchMaybe(s.F.get(iid), {
        Just: (frameBucket: Stack<Frame>) => {
            ...
            let sP2: State = setC(sP1, frameP.savedContext);
            return sP2;
        },
        Nothing: () => {
...
export function WInvokeFunPre(
    ...
    if (isExternal) {
        ...
        // Native callbacks into instrumented code arrive with raw runtime
        // arguments.  Treat the callback body as internal, but force one
        // wrapping pass here so local parameter reads observe wrapped values.
        if (s.frame.callType == CallType.NativeCall) {
            let sw = setCallType(s1, CallType.InternalCall);
            let wrappedBaseState;
            let wrappedBase;
            if (isMethod) {
                [wrappedBaseState, wrappedBase] = F.eitherThrow(wrap(sw, base));
            } else {
                [wrappedBaseState, wrappedBase] = [sw, base];
            }
            let si = wrappedBaseState;
            let wrappedArgs: Wrapped[] = [];
            for (let i = 0; i < args.length; i++) {
                let argWrapped;
                [si, argWrapped] = F.eitherThrow(wrap(si, args[i]));
                wrappedArgs.push(argWrapped);
            }
            let sP = saveFrame(si, CallType.InternalCall, iid);
            return F.Left([sP, [uf, wrappedBase as Unwrapped, wrappedArgs as Unwrapped[]]]);
        }
...
export function WFunctionEnter(
    ...
    // DynaJS can enter internal functions implicitly via getters/callbacks
    // without a matching invokeFunPre. In that case, preserve both the caller
    // frame and the pending expression context before starting the callee.
    if (getIDS(s).length() != 0 || s.C != Context.Unset) {
        let s1 = saveFrame(s, callType, -1);
        let s1P = setC(s1, Context.Unset);
        let s2 = F.eitherThrow(WPre(s1P));
        return F.Left(s2);
    } else {
...
export function WSuspend(
    s0: State,
    value: any
): Either<[State, [Unwrapped]], Error> {
    let s = F.eitherThrow(WPre(s0));
    F.assert(getIDS(s).length() == 0, 'WSuspend: IDS must be empty at start');
    let [s2, unwrapped] = F.eitherThrow(unwrap(s, value));
    return F.Left([s2, [unwrapped]]);
}

export function WResume(
    s0: State,
    value: any
): Either<[State, [Wrapped]], Error> {
    let [s1, wrapped] = F.eitherThrow(wrap(s0, value));
    return F.Left([s1, [wrapped]]);
}
