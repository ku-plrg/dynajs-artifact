...
const testRoot = path.join(repoRoot, "tests", "dynajs-analysis");
const dynajs = path.join(repoRoot, "lib", "dynajs", "dynajs");
const analysis = path.join(repoRoot, "src", "vendor", "NodeMedicAnalysis.mjs");

if (!fs.existsSync(analysis)) {
  console.error(chalk.red(`missing built analysis at ${analysis}. Run \`node scripts/build-nodemedic-analysis.mjs\` first.`));
  process.exit(2);
}
...
const buckets = [
  { label: "unit", dir: testRoot,                        required: true  },
...
const env = {
  ...process.env,
  DYNAJS_HOME: path.join(repoRoot, "lib", "dynajs"),
...
for (const { label, dir, required } of buckets) {
  ...
  for (const file of files) {
    ...
    if (required) {
      ...
      if (ok) {
        goalAchieved++;
        console.log(`${chalk.cyan("ACHIEVED")} [${label}] ${rel}  ${chalk.gray("(consider moving to tests/dynajs-analysis/unit/)")}`);
      } else {
        goalPending++;
