...
SHORT=p:,v:,m:,V::,f:,h
LONG=package:,version:,mode:,verdaccio::,flags:,help,batch-size:,stop-on-1st-exploited,timeout:
...
do
  case "$1" in
    ...
    -V | --verdaccio )
      ...
      shift 2
      ;;
    # --batch-size: the number of entrypoints to be processed at one time
    # default to 1 if not specified, meaning all entrypoints will be processed sequentially
    --batch-size )
      batch_size="$2"
      shift 2
      ;;
